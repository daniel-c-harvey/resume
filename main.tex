...
\skillsheader{Technical Skills Summary}
    \skillsitem{Programming Languages \& ISAs}{C\#, VB.NET, C++, Python, Java, TypeScript/JavaScript, Transact-SQL, C, HTML5, CSS3, PHP, Lua, LaTeX, Matlab, Mermaid, Verilog, VHDL, ARM Assembly (Thumb-2), RISC-V}
    ...
    \skillsitem{Cloud \& DevOps}{Azure DevOps, Team Foundation (TFS), BitBucket, Azure Pipelines, GitHub Actions, Digital Ocean Droplets, AWS EC2, Git, TFVC}
    ...
    \skillsitem{Tools \& Environments}{Visual Studio, Rider, DataGrip, VS Code, PyCharm, WebStorm, DataGrip, Cursor, SQL Server Management Studio, CLion, PhpStorm, IntelliJ, Eclipse, Postman/Insomnia, Bash, Vim, Claude Code}
...
\role{Application Architect}{October 2024 -- Present}{Cerebellum Softworks (Self-Employed)}{}{.NET 10/9/8, C\#, Entity Framework Core, ASP.NET Core with Identity, Node.js, TypeScript, Blazor Server \& Web Assembly, MudBlazor, PostgreSQL, SQLite, Google Cloud, Azure, Digital Ocean Droplets, Rider, DataGrip, Visual Studio, VS Code, Git, NUnit, Claude Code, Cursor}

\product{Babelfish Real-Time Captioning \& Translation}{Maintenance \& Enhancement Stage}{Implemented and operated real-time captioning and translation application for live technical conference events, utilizing Google Cloud gRPC Services, ASP.NET Core, and Blazor Server to deliver seamless multilingual support and accessibility features for diverse international audiences.}
   ...
   \achievement{Implemented real-time captioning by implementing efficient data streaming techniques and RT pipeline processing, yielding a total processing latency of <2 seconds from speech input to caption display}
   \achievement{Developed user-friendly interface with MudBlazor components, ensuring responsive monitoring and control for the system operator and clean presentation for event  attendees}
   \achievement{Operated the system across two TCB Council Workshops --- the four-day Fall 2025 event (English, Japanese, Korean, and Chinese; 28+ hours of technical sessions) and the three-day April 2026 event (scaled to 14 simultaneous languages over 21 hours, delivering 256,000+ translations at a 99.93\% success rate with $\sim$1-second end-to-end latency and zero downtime, outages, or interruptions)}
   \achievement{Crafted professional dynamics processing algorithms and controls to enhance audio clarity for speech recognition, including noise gating, upward expansion, compression, look-ahead limiting, and gain trimming}
\productend
...
\role{Software Developer}{July 2021 -- October 2024}{Enmark Systems, Inc.}{Ann Arbor, MI}{.NET Framework 4.8, VB.NET, C\#, MS SQL Server, Azure DevOps, WinForms, WPF, SOLID Principles, SOA, SteelXML, AISC Standards, Agile/Scrum, Visual Studio, DataGrip, SQL Server Management Studio, TFS, Git, Insomnia, AvaTax, Vertex}
...
\role{Backend Software \& Systems Engineer}{August 2020 -- June 2023}{RancUp}{Livonia, MI}{.NET Core 2.1, C\#, ASP.NET Core, Azure Virtual Machines, IIS, MS SQL Server, Entity Framework Core, RESTful APIs, Transact-SQL, Xamarin, JavaScript, Visual Studio, SQL Server Management Studio, Git, Postman}
...
\role{Software Developer Intern}{January 2016 -- June 2021}{Enmark Systems}{Ann Arbor, MI}{Agile/Waterfall SDLC, .NET Framework 3.0 - 4.5, VB.NET, Team Foundation Server, Transact-SQL, MS SQL Server 2008 - 2016, Visual Studio, WinForms, Windows Services, DataGrip, SQL Server Management Studio, Crystal Reports, Insomnia}
...
\project{BreakChop Step-Sequenced Audio Effect Plugin}{C++20, JUCE 8, VST3, CMake, juce::dsp, OpenGL, Real-Time Audio Processing, Lock-Free Concurrency, APVTS, DAW Transport Sync}[break\_chop]
    \achievement{Architected a tempo-locked signal pipeline (Pre-FX $\rightarrow$ Capture $\rightarrow$ Step FX $\rightarrow$ Crossfade $\rightarrow$ Post-FX $\rightarrow$ Dry/Wet) for a 32-step reorderable sequencer supporting up to four effects per step plus independent global Pre/Post FX chains, delivered as a cross-platform JUCE 8 VST3 and standalone plugin with OpenGL-rendered effect and waveform visualizers}
    \achievement{Engineered strict audio/message-thread separation using \texttt{std::atomic} UI feedback and a resolved-POD snapshot pattern for runtime macro configuration, eliminating audio-thread allocation and reference-counted types on the real-time path while tolerating at most a single block of stale modulation values}
    \achievement{Crafted a stutter effect with exponentially-spaced repeat slots (geometric-sum normalized so accelerando/decelerando preserves total segment length), fractional-interpolation pitch shifting ramped across repeat index, in-place reverse via post-capture buffer flip, quadratic attack/release envelope with floor compensation and per-slot decay, and equal-power stereo spread that alternates L/R with intensity growing per repeat}
    \achievement{Designed an O(1)-indexed FX topology service that links same-type effects across steps via \texttt{shared\_ptr} parameter pooling, combined with a macro modulation engine supporting function-generator shapes (division, skew, smoothing, phase, bias) routed to global, pooled, or per-slot destinations, with a strategy-based gate pattern library (Uniform, Syncopated, Dotted, Triplet) open for extension}
\projectend
