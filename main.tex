...
\product{Skipper ERP }{Design \& Development Stage}{Architecting comprehensive marina management system for commercial operations, featuring slip reservations, vessel \& customer management, self-service booking, maintenance scheduling, billing automation, and financial reporting}
...
\product{RancUp Mobile App \& Website}{Maintenance \& Enhancement Stage}{Enhanced and maintained high-performance backend services for fantasy football platform, supporting 500+ concurrent users during peak season.}
    ...
    \achievement{Orchestrated manual app publications \& deployments to Azure Virtual Machines, coordinating code integration from multiple developers and executing controlled releases with comprehensive testing protocols and rollback strategies during critical peak season periods}
    \achievement{Designed and implemented dynamic player re-ranking algorithms with locked player logic, enabling users to reorder rankings based on custom methodologies while maintaining data integrity across weekly and full-season statistical calculations}
\productend
...
\product{Eniteo ERP}{Maintenance \& Enhancement Stage}{Contributed to the development and enhancement of specialized ERP solutions for metals service centers, gaining foundational expertise in steel industry operations and business process automation across multiple functional areas.}
    ...
    \achievement{Led onboarding and training programs for 8+ new developer hires, creating comprehensive documentation and mentoring protocols that significantly reduced ramp-up time}
...
\project{Node Media Server}{Node.js, Express.js, Linux ext4 filesystem, Digital Ocean Droplet}[snailbird-content]
    ...
    \achievement{Developed RESTful API with Base64 encoding, MIME type handling, and API key authentication middleware, enabling secure media upload/retrieval across distributed web applications}
    \achievement{Implemented custom StructuralMap abstract data structure and command-line interface for flexible deployment configurations, supporting scalable integration with client projects requiring centralized media asset delivery}
\projectend
...
\twostudylist
{
    ...
    \course{Software Engineering}{Software development lifecycle, requirements analysis and formal documentation, use case modeling, design patterns and architecture, and testing methodologies}
    \course{Operating Systems}{C systems programming, process management and scheduling algorithms, file system implementation, memory management, inter-process communication, and multithreading synchronization}
    \course{Distributed Systems/Cloud Computing}{Network programming with Python, client-server architecture, socket programming and TCP/IP protocols, AWS cloud computing with EC2 instances, and fault-tolerant distributed applications}
    \course{Database Systems}{Dynamic hash-based database implementation, indexing optimization, CRUD operations with SQL, record management systems, and DBMS-powered web app design with PHP}
    \course{Algorithm Design and Analysis}{Graph algorithms (BFS, DFS, cycle detection), advanced sorting with empirical analysis, string pattern matching (Boyer-Moore), complexity theory, and Red-Black and 2-3 tree implementations}
    \course{Systems Programming}{Unix/Linux systems programming in C, functional programming with Haskell (lazy evaluation, algebraic types), shell scripting, and system utility implementation}
    \course{Theory of Computation}{Formal languages and automata theory, computational complexity analysis, Turing machines, decidability problems, and theoretical foundations of computer science}
    \studyend
}{
    \study{Computer Engineering}
    \course{Real-Time Computer Systems}{ARM Cortex-M3/M4 programming, real-time operating systems (RTOS), digital signal processing applications, and advanced peripheral integration; final project implemented a multi-mode digital wavetable function generator on the ARM Cortex M3 using CMSIS-RTOS for user interface integration with DAC hardware}
    \course{Computer Architecture \& Organization}{MIPS processor architecture and instruction set, CPU design (single-cycle, multi-cycle, pipelined), pipeline hazards and forwarding, cache memory hierarchy, microcoding, and ARM Cortex-M3 analysis}
    \course{Digital Systems II}{Advanced FPGA programming with Verilog HDL, complex combinational and sequential circuit design, finite state machine implementation, and system-on-programmable-chip (SoPC) development}
    \course{Microcomputers II}{Advanced microcontroller programming with PIC32, MIPS32 architecture, serial communication protocols (UART, SPI, I2C, CAN), Bluetooth Low Energy, and real-time systems design; final project implemented a web-API powered card payment system on an ESP32 into a functioning vending machine}
    \course{Digital Systems I}{Digital logic design fundamentals, Boolean algebra and logic minimization, combinational and sequential circuits, VHDL hardware description language, and FPGA implementation}
    \course{Microcomputers I}{Embedded systems design with PIC32, PIC24, and Freescale microcontrollers, assembly language programming, interrupt handling, real-time programming, and I/O peripheral control}
    \studyend
}
